...
Port of code-security-review/scripts/lib/report.py adapted for context-optimization
finding shape (category instead of OWASP/endpoint).
"""
...
def render_markdown(findings: list[Finding]) -> str:
    ...
    for f in findings:
        ...
        by_category[str(f.get("category", "unknown"))].append(f)

    lines: list[str] = []
    lines.append("# Context Optimization — Audit Report")
    lines.append("")
    ...
    if not findings:
        lines.append("> No findings recorded. Either the context passes the audit or no scripts were run.")
        lines.append("")
        return "\n".join(lines)

    # --- Verdict ---
    if counts.get("critical", 0) > 0:
        verdict = "REWORK — critical findings present, optimization plan needs immediate adoption"
    elif counts.get("high", 0) > 0:
        verdict = "REVIEW — high-severity findings require targeted remediation"
    elif counts.get("medium", 0) > 0:
        verdict = "TUNE — medium-severity findings warrant attention"
    ...
    for cat in sorted(by_category.keys()):
        ...
        lines.append(f"| {label} | {len(by_category[cat])} |")
    lines.append("")
    ...
    for sev in SEVERITY_ORDER:
        ...
        for f in sev_findings:
            lines.append(f"### `{f.get('id', '?')}` — {f.get('target', '?')}")
            lines.append("")
            lines.append(f"- **Phase:** `{f.get('phase', '?')}`")
            cat = str(f.get("category", "—"))
            lines.append(f"- **Category:** {CATEGORY_LABEL.get(cat, cat)}")
...
def render_json_summary(findings: list[Finding]) -> str:
    counts: Counter[str] = Counter(str(f.get("severity", "info")) for f in findings)
    by_category: Counter[str] = Counter(str(f.get("category", "unknown")) for f in findings)
    summary: dict[str, Any] = {
        "total":  len(findings),
        "counts": {sev: counts.get(sev, 0) for sev in SEVERITY_ORDER},
        "by_category": dict(by_category),
        "verdict_rework": counts.get("critical", 0) > 0,
        "verdict_review": counts.get("high", 0) > 0,
    }
