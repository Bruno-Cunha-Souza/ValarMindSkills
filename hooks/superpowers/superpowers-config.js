#!/usr/bin/env node
// superpowers — shared configuration resolver
//
// Inspired by obra/superpowers (MIT, Copyright 2025 Jesse Vincent).
// See THIRD_PARTY_NOTICES.md.
//
// Resolution order for default mode:
//   1. SUPERPOWERS_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/superpowers/config.json (any platform, if set)
//      - ~/.config/superpowers/config.json (macOS / Linux fallback)
//      - %APPDATA%\superpowers\config.json (Windows fallback)
//   3. 'off'  ← critical difference from caveman; superpowers stays disabled
...
function getDefaultMode() {
  const envMode = process.env.SUPERPOWERS_DEFAULT_MODE;
...
function safeWriteFlag(flagPath, content) {
  try {
    ...
    const tempPath = path.join(flagDir, `.superpowers-active.${process.pid}.${Date.now()}`);
